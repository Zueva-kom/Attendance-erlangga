...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ========================================================
    // 2. LOGIKA WAKTU JENDELA ABSEN (DIPAKSA IN & MASUK)
    // ========================================================
    ...
    // Membuat string tanggal lokal (YYYY-MM-DD)
    ...
    // Ambil jam & menit sekarang secara riil untuk dicatat di database
    ...
    // Paksa status agar selalu masuk tepat waktu untuk demo
    const dbStatus = "IN"; 
    const responStatusNodeMCU = "MASUK"; // Selalu "MASUK" (tidak akan "TERLAMBAT")
    ...
    if (resultCek.rows.length > 0) {
      ...
      // Supaya gampang ditunjukkan berulang kali saat demo, kita ijinkan update waktu terbarunya
      const queryUpdate = `
        UPDATE presensis 
      ...
      return res.status(200).json({ status: responStatusNodeMCU, name: namaSiswa, message: "Absen masuk diperbarui" });
    } else {
      // DATA BELUM ADA HARI INI -> INSERT BARU
      const queryInsert = `
